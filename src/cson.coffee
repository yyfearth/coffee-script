...
BANNER = '''
  Usage: cson [options] path/to/script.cson
...
SWITCHES = [
  ['-o', '--output [DIR]',      'set the output directory for compiled JSON']
  ['-e', '--eval',              'pass a string from the command line as input']
  ['-p', '--print',             'print out the compiled JSON']
  ['-s', '--stdio',             'listen for and compile scripts over stdio']
  ['-w', '--watch',             'watch scripts for changes and rerun commands']
  ['-b', '--beautify',          'beautify compiled JSON conflict with --raw']
  [      '--space [SPACE]',     'beautify compiled JSON with space need --beautify']
  ['-r', '--raw',               'return row compiled JSON without check and compact']
  [      '--safe',              'do not use eval to parse compiled JSON']
...
# Run `cson` by parsing passed options and determining what action to take.
...
# is passed, recursively compile all '.cson' extension source files in it
# and all subdirectories.
compilePath = (source, topLevel, base) ->
  fs.stat source, (err, stats) ->
    throw err if err and err.code isnt 'ENOENT'
    if err?.code is 'ENOENT'
      if topLevel and source[-7..] isnt '.cson'
    ...
    else if topLevel or path.extname(source) is '.cson'
...
procJSON = (code) ->
  from = code.indexOf '{'
  to = code.lastIndexOf '}'
  throw 'Invalid JSON compiled, cannot find { or }' if from < 0 or to < 0
  json = code[from..to]
...
parseOptions = ->
  ...
  o.print       = !!  (o.print or (o.eval or o.stdio))
...
compileOptions = (filename) ->
  {filename, bare: yes, runtime: opts.runtime}
...
version = ->
  printLine "CSON with eXtraCoffeeScript version #{CoffeeScript.VERSION}"
