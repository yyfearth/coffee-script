(function() {
  ...
  SWITCHES = [['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['--header', 'use a string as a head when compile is on'], ['-x', '--imports', 'enable import feature'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive CoffeeScript REPL'], ['-j', '--join [FILE]', 'concatenate the source CoffeeScript before compiling'], ['-l', '--lint', 'pipe the compiled JavaScript through JavaScript Lint'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['-o', '--output [DIR]', 'set the output directory for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-t', '--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  xoptions = function() {
    ...
    o = opts;
    if (o.header != null) {
      if ((o.header != null) && !o.compile) {
        throw 'use header only with compile';
      }
      o.header = o.header === 'on' ? true : o.header === 'off' ? false : o.header;
    } else {
      o.header = o.compile;
    }
    o.imports = !!o.imports;
  ...
  version = function() {
    return printLine("eXtraCoffeeScript version " + CoffeeScript.VERSION);
  };

}).call(this);

// Generated by eXtraCoffeeScript 1.5.0-pre
