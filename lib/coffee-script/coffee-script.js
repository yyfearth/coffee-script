...
(function() {
  var EXTENSIONS, Lexer, RESERVED, chk_file, compile, e, fs, iced, imports, isCoffeeFile, lexer, parser, path, vm, _i, _j, _len, _len2, _ref,
  ...
  exports.imports = imports = function(code, options) {
    var cwd, filename, imported, parse, regex, regex_g;
    ...
    return (parse = function(code, filename) {
      return code.replace(regex_g, function(match) {
        var dir, f, import_code, is_js, rmk, _filename, _is_js;
        match = match.match(regex);
        dir = filename ? path.dirname(fs.realpathSync(filename)) : cwd;
        _filename = f = path.resolve(dir, match[1]);
        if (!chk_file(_filename)) _filename = f + '.coffee';
        if (!chk_file(_filename)) _filename = f + '.iced';
        if (!chk_file(_filename)) _filename = f + '.js';
        ...
        rmk = /\.js$/.test(filename) ? "/* imported " + _filename + " */\n" : "### imported " + _filename + " ###\n";
        return rmk + parse(import_code, _filename);
      });
    })(code, filename);
  };
  ...
  exports.compile = compile = function(code, options) {
    var header, js, merge;
    if (options == null) options = {};
    if (options.extra) code = imports(code, options);
