...
(function() {
  var BANNER, CoffeeScript, EventEmitter, SWITCHES, compileOptions, compilePath, compileScript, compileStdio, exec, fs, helpers, iced, loadRequires, notSources, optionParser, optparse, opts, outputPath, parseOptions, path, printLine, printWarn, procJSON, removeSource, runtime_modes_str, sourceCode, sources, spawn, timeLog, unwatchDir, usage, version, wait, watch, watchDir, watchers, writeJs, _ref;
  ...
  procJSON = function(code) {
    ...
    from = code.indexOf('{');
    to = code.lastIndexOf('}');
    if (from < 0 || to < 0) throw 'Invalid JSON compiled, cannot find { or }';
    json = code.slice(from, to + 1 || 9e9);
    if (opts.raw && !opts.safe) return json;
    try {
      obj = JSON.parse(json);
    } catch (e) {
      if (opts.safe) {
      ...
      try {
        obj = eval(code);
        if (obj == null) throw 'eval return null';
      } catch (e) {
  ...
  parseOptions = function() {
    ...
    if (o.space != null) {
      if (!o.beautify) throw 'space need beautify is on';
      if (/^\d+$/.test(o.space)) o.space = o.space >>> 0;
    } else {
      o.space = 4;
    }
    o.raw = !!o.raw;
    if (o.help == null) o.help = process.argv.length < 3;
    if (o.beautify && o.raw) throw 'beautify conflict with raw';
    sources = o["arguments"];
